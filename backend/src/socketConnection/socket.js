...
import { DisconnectedStudyRoom, JoinedStudyRoom, userLeavesRoom } from "../middlewares/studytime.count.js";
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ roomId, name, email, hostStatus }) => {
      ...
      if (!hostStatus) {
        ...
        if (roomId == room) {
          ...
          JoinedStudyRoom(email, socket.id, roomId);
        } else {
    ...
    socket.on("disconnect", () => {
      ...
      if (email) {
        ...
        DisconnectedStudyRoom(email);
        userLeavesRoom(email);
      }
    });
  });
};
